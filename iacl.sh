...
useradd debora -m -s /bin/bash -p $(openssl passwd -6 senha123) -G GRP_ADM
...
useradd amandaa -m -s /bin/bash -p $(openssl passwd -6 senha123) -G GRP_SEC
useradd rogerio -m -s /bin/bash -p $(openssl passwd -6 senha123) -G GRP_SEC

echo "Especificando permissõesx dos diretórios..."
